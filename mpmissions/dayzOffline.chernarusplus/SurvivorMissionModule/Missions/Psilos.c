class PsilosMission extends SurvivorMissions
{
	...
	void PsilosMission()
	{
		...
		m_MissionMessage2 = "Зараз я експериментую з психоактивним інгредієнтом псилоцибіном на заражених диких тваринах. Тварини демонструють цікаву поведінку через 30 хвилин після прийому всередину. Я поки не можу багато сказати, але для моїх експериментів мені потрібно більше грибів.";
	...
	void SpawnAIs()
	{
		...
		else
		{
			...
			{
				if (k == 0)
					RandomInfected = "ZmbM_PolicemanFat";
				else if (k == 1)
					RandomInfected = "ZmbF_PoliceWomanNormal";
				else if (k == 2)
					RandomInfected = "ZmbM_PolicemanSpecForce";
				else if (k == 3)
					RandomInfected = "ZmbM_PolicemanFat";
				else
	...
	void ExtendMission()
	{
		...
		if (EventsWorldData.GetBuildingsAtLoc("Land_Village_PoliceStation_Enoch", m_MissionDescription[3], ExtendedPosList))
		{
			...
			Print("[SMM] Can't find MissionBuilding Village Police Station in " + m_MissionDescription[3] + "!");
	}
